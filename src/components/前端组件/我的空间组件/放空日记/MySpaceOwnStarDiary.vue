...
export default {
  ...
  methods: {
    ...
    currentChange(currentIndex){
      ...
      this.getUserSpaceDiaryUserStarOrderByCreateTime(currentIndex)
    },
    // justToDiaryDetail(row) {
    //   // console.log(row.id);
    //   this.$router.push({name: 'DiaryPageDiaryDetailDisplay' , query:{diaryId: row.id}});
    // },
    getUserSpaceDiaryUserStarOrderByCreateTime(currentIndex) {
      axios.get('/getUserSpaceDiaryUserStarOrderByCreateTime?currentIndex=' + currentIndex).then(response => {
    ...
    handleDelete(index , row) {
      ...
      axios.post('/cancelObjStar' , {
        ...
        this.getUserSpaceDiaryUserStarOrderByCreateTime(1);
        this.currentPage = 1;
      })
    }
  },
  mounted() {
    ...
    this.getUserSpaceDiaryUserStarOrderByCreateTime(this.currentPage);
